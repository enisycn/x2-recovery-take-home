...
Training also uses two published exploration ideas. HumanUP's Stage-I standing-pose mixture starts at 50% and reaches zero after 8.192 million aggregate simulator transitions. HoST's official cross-robot guidance scales its upward pull to about 60% of robot weight and enables it only after the trunk is near vertical. For the 41.966521 kg X2 this is 247.015 N when `-g_b,z≥0.80`, reaching zero after 12.288 million aggregate transitions. Both curricula are disabled in play/evaluation; in the 3,000-environment final run they end at about iterations 85 and 128, leaving roughly 272 of 400 iterations fully unassisted. Exact equations, adaptations and numerical checks are in [the formula audit](docs/formula_audit.md); every source-to-code connection is in [the evidence map](docs/evidence.md).

## PPO training and evaluation

The final measured run uses 3,000 environments for 400 PPO iterations, with 32 steps per environment and 38.4 million simulator transitions in total. A five-iteration smoke run measured about 96,000 transitions per 9.6-second collection phase while using about 5 GB of GPU memory. The run uses seed 42, ELU MLPs `[512, 256, 128]`, learning rate `3e-4`, clip `0.2`, discount `0.99`, GAE lambda `0.95`, five learning epochs and four mini-batches. Earlier branches with incorrect frame, floor or assistance physics are diagnostic only and are excluded from the final result.

```bash
./scripts/train_isaac.sh --max_iterations 400 --num_envs 3000 --seed 42 --device cuda:0
