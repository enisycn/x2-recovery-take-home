> Historical experiment only. Its curriculum-only diagnosis and claimed continuous reference-2 success were superseded by the 22 September no-training audit. Commands below target old checkpoints, not simple_v2.

# AgiBot X2 ground recovery

**22 September diagnostic update:** The [no-training review](no_training_review_2026-09-22.md)
...
See [the validation record](../reports/validation.md) for commands and observed outputs, [the root-cause record](root_cause.md) for environment findings, and [the evidence map](evidence.md) for the research behind each design choice.
