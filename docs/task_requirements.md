...
| RL experiment and checkpoint/plot | Final PPO run: 3,000 environments, 400 optimizer iterations and 38.4 million simulator transitions; final checkpoint, scalar CSV, reward plot and strict evaluation are retained |
...
| Five simulation episodes | Final Isaac checkpoint evaluated on seeds 101–105 from reproducible perturbed back-lying starts: 0/5, with per-check metrics and failure analysis in `reports/isaac_evaluation.json` and `reports/validation.md` |
